...
async function scrapeData(url, page){
    try {
        ...
        let price = $("#price > div > span.hide-content.display-inline-block-m > span > span.visuallyhidden").text();
        let seller = '';
        ...
        let outOfStock = '';
        let checkOutOfStock = $('#blitzitem-container > div > div > div > div > b');
...
router.post('/product/new', isAuthenticatedUser, (req, res) => {
    ...
    Product.findOne({sku: sku})
        .then(product => {
            if(product){
                req.flash('error_msg', 'Product already exists in the database');
